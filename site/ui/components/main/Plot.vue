<template>
  ...
  <div class='plot top justify-content-center'>
...
export default {
  ...
  mounted () {
    ...
    Plotly.plot('plot', data, layout).then(plot => {
      this.plot = plot;
      // Plotly.downloadImage(plot, {format: 'png', width: 2000, height: 1500, filename: 'newplot'});
    });
  },
  methods: {
    downloadImage () {
      Plotly.downloadImage(this.plot, {format: 'png', width: 2000, height: 1500, filename: 'newplot'});
    },
  },
};
</script>
